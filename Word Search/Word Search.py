...
# Just a blank canvas to put the board on, this was mainly for doing further things down the road
canvas = Canvas(root)
...
for row in range(0, size):
    for col in range(0, size):
        if a[row][col] == "-":
            ...
            spot = Label(canvas, text = a[row][col], font=("Arial", 24), width = 2)
        else:
            spot = Label(canvas, text = a[row][col], font=("Arial", 24), width = 2, bg = "hot pink")
